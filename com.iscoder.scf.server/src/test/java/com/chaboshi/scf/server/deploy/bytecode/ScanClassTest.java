...
public class ScanClassTest {
    ...
    for (SessionBean sb : ci.getSessionBeanList()) {
      ...
      while (it.hasNext()) {
        ...
        if (i == 0) {
          ...
          Assert.assertEquals("com.bj58.spat.servicedemo.components.RestService", entry.getValue());
        } else if (i == 1) {
          Assert.assertEquals("NewsService", entry.getKey());
          Assert.assertEquals("com.bj58.spat.servicedemo.components.NewsService", entry.getValue());
        }
        i++;
      }
    }
  ...
  public void testGetContractClassInfos() throws Exception {
    ...
    for (int i = 0; i < ciList.size(); i++) {
      if (i == 0) {
        Assert.assertEquals("com.bj58.spat.servicedemo.contract.INewsService", ciList.get(i).getCls().getName());
      }
    }
  }
  ...
  public void testGetBehaviorClassInfos() throws Exception {
    ...
    for (int i = 0; i < ciList.size(); i++) {
      if (i == 0) {
        ...
        Assert.assertEquals("com.bj58.spat.servicedemo.components.NewsService", ciList.get(i).getCls().getName());
      } else if (i == 1) {
        Assert.assertEquals("RestService", ciList.get(i).getLookUP());

        Assert.assertEquals("com.bj58.spat.servicedemo.components.RestService", ciList.get(i).getCls().getName());
      }
