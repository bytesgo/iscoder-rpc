...
import com.iscoder.scf.server.deploy.bytecode.ClassInfo.ParamInfo;
import com.iscoder.scf.server.deploy.bytecode.ContractInfo.SessionBean;
import com.iscoder.scf.server.deploy.hotdeploy.DynamicClassLoader;

public class ScanClassTest {

  private static String jarPath = "E:\\javaproject\\iscoder.scf.server\\lib";
  ...
  public void testGetContractInfo() throws Exception {
    ...
    for (SessionBean sb : ci.getSessionBeanList()) {
      ...
      Assert.assertEquals("com.iscoder.spat.servicedemo.contract.INewsService", sb.getInterfaceName());
      ...
      while (it.hasNext()) {
        ...
        if (i == 0) {
          ...
          Assert.assertEquals("com.iscoder.spat.servicedemo.components.RestService", entry.getValue());
        } else if (i == 1) {
          Assert.assertEquals("NewsService", entry.getKey());
          Assert.assertEquals("com.iscoder.spat.servicedemo.components.NewsService", entry.getValue());
        }
        i++;
      }
    }
  ...
  public void testGetContractClassInfos() throws Exception {
    ...
    for (int i = 0; i < ciList.size(); i++) {
      if (i == 0) {
        Assert.assertEquals("com.iscoder.spat.servicedemo.contract.INewsService", ciList.get(i).getCls().getName());
      }
    }
  }
  ...
  public void testGetBehaviorClassInfos() throws Exception {
    ...
    for (int i = 0; i < ciList.size(); i++) {
      if (i == 0) {
        Assert.assertEquals("NewsService", ciList.get(i).getLookUP());

        Assert.assertEquals("com.iscoder.spat.servicedemo.components.NewsService", ciList.get(i).getCls().getName());
      } else if (i == 1) {
        Assert.assertEquals("RestService", ciList.get(i).getLookUP());

        Assert.assertEquals("com.iscoder.spat.servicedemo.components.RestService", ciList.get(i).getCls().getName());
      }
