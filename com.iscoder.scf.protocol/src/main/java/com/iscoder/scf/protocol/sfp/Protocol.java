...
import com.iscoder.scf.common.utils.DESCoderUtil;
import com.iscoder.scf.protocol.compress.CompressBase;
...
import com.iscoder.scf.protocol.serializer.AbstractSerializer;
...
public class Protocol {
  ...
  public static Protocol fromBytes(byte[] data, boolean rights, byte[] desKey) throws Exception {
    ...
    if (p.getSDPType().getNum() != 5 && rights && desKey != null) {
      sdpData = DESCoderUtil.getInstance().decrypt(sdpData, desKey);// DES解密数据
    }
  ...
  public byte[] toBytes(boolean rights, byte[] desKey) throws Exception {
    ...
    if (this.getSDPType().getNum() != 5 && rights && desKey != null) {
      sdpData = DESCoderUtil.getInstance().encrypt(sdpData, desKey);// DES加密数据
    }
