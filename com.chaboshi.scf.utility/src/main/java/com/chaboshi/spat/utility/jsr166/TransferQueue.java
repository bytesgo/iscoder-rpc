...
public interface TransferQueue<E> extends BlockingQueue<E> {
   ...
   * More precisely, transfers the specified element immediately if there exists
   * a consumer already waiting to receive it (in {@link #take} or timed
   * {@link #poll(long,TimeUnit) poll}), else waits until the element is
   * received by a consumer.
   *
   * @param e the element to transfer
   * @throws InterruptedException if interrupted while waiting, in which case
   *           the element is not left enqueued
   * @throws ClassCastException if the class of the specified element prevents
   *           it from being added to this queue
   * @throws NullPointerException if the specified element is null
   * @throws IllegalArgumentException if some property of the specified element
   ...
   * a consumer already waiting to receive it (in {@link #take} or timed
   * {@link #poll(long,TimeUnit) poll}), else waits until the element is
   * received by a consumer, returning {@code false} if the specified wait time
   * elapses before the element can be transferred.
   *
   * @param e the element to transfer
   ...
   * via {@link #take} or timed {@link #poll(long,TimeUnit) poll}. The return
   * value is an approximation of a momentary state of affairs, that may be
   * inaccurate if consumers have completed or given up waiting. The value may
   * be useful for monitoring and heuristics, but not for synchronization
   * control. Implementations of this method are likely to be noticeably slower
